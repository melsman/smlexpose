...
*)
    ...
    fun pp_typ t =
        let fun arg s t acc = s :: "(" :: pp (t, ")"::acc)
            and pp (t, acc) =
                ...
                | Tvector t => arg "vector" t acc
                | Tlist0 t => arg "list0" t acc
	...
	debug "word8" (PU
	{pickler = fn v => fn s => S.outw8 (v,s),
	 unpickler = fn (s,hce) => let val (v,s) = S.getw8 s
			           in (v, (s,hce))
			           end,
	 hasher = fn v => hashComb (fn p => hashAdd (Word.fromLarge(Word8.toLarge v)) p),
	 eq = op =,
	 typ = Tword8})
...
*)
	...
	(shareGen o debug "string") string0
...
*)
    ...
    val real =
	debug "real"
	(convert0 (fn s => PackRealLittle.fromBytes(Byte.stringToBytes s),
		   fn r => Byte.bytesToString(PackRealLittle.toBytes r))
	          string0)
    ...
    fun vectorHash (elemHash: 'a hasher) (v:'a vector) (acc: word*int) : word*int =
	let val sz = Vector.length v
	    val sz = if sz > maxLength then maxLength else sz
	    fun loop (n,a) =
		if n >= sz then a
		else loop (n+1,elemHash (Vector.sub(v,n)) a)
	in loop (0,acc)
	end

    fun vectorEq eq (a,b) =
        Vector.length a = Vector.length b andalso
        EQUAL = Vector.collate (fn (a,b) => if eq (a,b) then EQUAL else LESS) (a,b)

    fun listHash (elemHash: 'a hasher) (v:'a list) (acc: word*int) : word*int =
        List.foldl (fn (e,a) => elemHash e a) acc v

    fun listEq eq (a,b) =
        List.length a = List.length b andalso
        EQUAL = List.collate (fn (a,b) => if eq (a,b) then EQUAL else LESS) (a,b)

    fun vectorGen (pu :'a pu) : 'a vector pu =
	let val hash_vector = newHashCount()
	in
	    debug "vector" (PU
	    {pickler = (fn (v:'a vector) => fn os =>
			   let val sz = Vector.length v
                               val os = S.outcw(Word.fromInt sz, os)
                           in Vector.foldl (fn (v,os) => pickler pu v os) os v
			   end),
	     unpickler = (fn (is,hce) =>
			  let val (sz,is) = S.getcw is
			      val sz = Word.toInt sz
                              val (es,is) = iter (fn (es,is) =>
                                                     let val (e,is) = unpickler pu is
                                                     in (e::es,is)
                                                     end) (sz,(nil,(is,hce)))
                          in (Vector.fromList (List.rev es), is)
			  end),
	     hasher = fn v => hashComb (fn acc => vectorHash (hasher pu) v acc),
	     eq = fn (a,b) => vectorEq (eQ pu) (a,b),
	     typ = Tvector(typ pu)})
	end

    fun listGen0 (pu :'a pu) : 'a list pu =
	let val hash_vector = newHashCount()
	in
	    debug "list0" (PU
	    {pickler = (fn (v:'a list) => fn os =>
			   let val sz = List.length v
                               val os = S.outcw(Word.fromInt sz, os)
                           in List.foldl (fn (v,os) => pickler pu v os) os v
			   end),
	     unpickler = (fn (is,hce) =>
			  let val (sz,is) = S.getcw is
			      val sz = Word.toInt sz
	     ...
	     hasher = fn v => hashComb (fn acc => listHash (hasher pu) v acc),
	     eq = fn (a,b) => listEq (eQ pu) (a,b),
	     typ = Tlist0(typ pu)})
	end
...
(*
    val real =
	debug "real"
	(convert0 (fn s => case Real.fromString s of
                            SOME r => r
                          | NONE => fail ("real.impossible: " ^ s),
		  Real.toString)
	 string0)
*)
